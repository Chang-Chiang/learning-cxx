...
struct Tensor4D {
    ...
    Tensor4D &operator+=(Tensor4D const &others) {
        ...
        unsigned int stride_other[4] = {others.shape[1] * others.shape[2] * others.shape[3], others.shape[2] * others.shape[3], others.shape[3], 1};

        for (unsigned int idx = 0; idx < size; ++idx) {
            unsigned int idxs[4];
            unsigned int tmp = idx;
            for (int d = 0; d < 4; ++d) {
                idxs[d] = tmp / stride_this[d];
                tmp %= stride_this[d];
            }
            unsigned int other_idx = 0;
            for (int d = 0; d < 4; ++d) {
                unsigned int od = (others.shape[d] == 1) ? 0 : idxs[d];
                other_idx += od * stride_other[d];
            }
